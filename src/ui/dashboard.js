(function initDashboard() {
  ...
  const els = {
    ...
    pageLabel: document.getElementById("paginationInfo") || document.getElementById("paginationLabel"),
  ...
  function renderChips() {
    ...
    if (state.chartHour) chips.push(["chartHour", `Hour: ${new Date(state.chartHour).toLocaleString()}`]);
    els.chips.innerHTML = chips.map(([key, label]) => `<button class="chip" data-key="${key}">${label} x</button>`).join("");
  }
  ...
  function renderTable() {
    ...
    for (const log of state.logs) {
      ...
      `);
      if (expanded) {
        const hasMeta = log.meta && typeof log.meta === "object" && Object.keys(log.meta).length > 0;
        const blocks = [];
        if (hasMore) {
          blocks.push(`<div class="detail-block"><div class="detail-label">Full message</div><pre class="detail-pre">${escapeHtml(fullMessage)}</pre></div>`);
        }
        if (hasMeta) {
          blocks.push(`<div class="detail-block"><div class="detail-label">Metadata</div><pre class="detail-pre">${escapeHtml(JSON.stringify(log.meta, null, 2))}</pre></div>`);
        }
        const detailHtml = blocks.length ? blocks.join("") : '<div class="detail-empty">No additional details</div>';
  ...
  function renderInsights(data) {
    ...
    if (!topSources.length) {
      ...
      els.topSources.innerHTML = topSources.map((source) => {
    ...
    if (!topErrors.length) {
      ...
      els.topErrors.innerHTML = topErrors.map((item) => {
        const compactMessage = previewText(item.message, 110);
        const sourceHint = item.sources?.length ? ` (${item.sources.length} source${item.sources.length > 1 ? "s" : ""})` : "";
    ...
    if (!spikes.length) {
      ...
      els.spikeHours.innerHTML = spikes.map((spike) => {
  ...
  function renderChart(data) {
    ...
    const chartHost = document.getElementById("chartWrapper")
      || document.getElementById("logChartContainer")
      || document.querySelector(".chart-wrap")
      || document.querySelector(".chart-container");
    const hasData = hourBuckets.some((bucket) => (Number(bucket.info || 0) + Number(bucket.warn || 0) + Number(bucket.error || 0) + Number(bucket.debug || 0)) > 0);
    ...
    const stackTotals = hourBuckets.map((bucket) => Number(bucket.info || 0) + Number(bucket.warn || 0) + Number(bucket.error || 0) + Number(bucket.debug || 0));
    const maxStack = Math.max(...stackTotals, 0);
    const yStep = maxStack <= 10 ? 1 : maxStack <= 50 ? 5 : maxStack <= 100 ? 10 : maxStack <= 500 ? 50 : 100;
  ...
  els.startDate.addEventListener("keydown", (event) => { if (event.key === "Enter") applyDraftDateRange(); if (event.key === "Escape") syncDraftInputsFromState(); });
  els.endDate.addEventListener("keydown", (event) => { if (event.key === "Enter") applyDraftDateRange(); if (event.key === "Escape") syncDraftInputsFromState(); });
  els.applyDate.addEventListener("click", applyDraftDateRange);
  els.cancelDate.addEventListener("click", () => { syncDraftInputsFromState(); setDateValidation(""); });
  ...
  els.pageSize.addEventListener("change", () => { syncStateFromInputs(); state.page = 1; fetchLogs(); });

  els.clear.addEventListener("click", () => {
    state.search = ""; state.level = ""; state.source = ""; state.startDate = ""; state.endDate = ""; state.chartHour = "";
    state.draftStartDate = ""; state.draftEndDate = "";
    els.search.value = ""; els.level.value = ""; els.source.value = "";
    setPickerValue(fpStart, "");
    setPickerValue(fpEnd, "");
    setPresetActive("");
    setDateValidation("");
